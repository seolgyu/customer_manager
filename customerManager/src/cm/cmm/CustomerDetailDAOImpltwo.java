...
public class CustomerDetailDAOImpltwo implements CustomerDetailDAOtwo{
   ...
   public List<CustomerDetailDTO> customerFindByName(String name, int page, int rows) {
      ...
      try {
         sql =  " SELECT * FROM ("
             ...
             + "        FROM customer c"
             + "        LEFT JOIN CUS_MILE cM ON c.CUS_ID = cM.CUS_ID"
   ...
   public List<YearlyMonthlyStatsDTO> getMonthlyBuyStats(String startYear){
    ...
    public List<CustomerDetailDTO> customerLoginHistory(String id, int page, int rows){
        ...
        try {
            sql = " SELECT LOGIN_ID, TO_CHAR(LOGIN_DATE, 'YYYY-MM-DD') LOGIN_DATE, CUS_ID"
            		+ " FROM ("
            		+ "    SELECT ROWNUM rn, LOGIN_ID, LOGIN_DATE, CUS_ID"
            		+ "    FROM ("
            		+ "        SELECT *"
            ...
            while (rs.next()) {
                ...
                dto.setReg(rs.getString("LOGIN_DATE"));
                dto.setName(rs.getString("CUS_ID"));
